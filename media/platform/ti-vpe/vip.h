...
/*
 * The vip_shared structure contains data that is shared by two SLICEs
 */
struct vip_shared {
	struct list_head	list;
	struct resource		*res;
...
//SLICE is a vip_dev
struct vip_dev {
	struct v4l2_device	v4l2_dev; //each SLICE registers a v4l2 device
	struct v4l2_ctrl_handler ctrl_handler;
	struct platform_device *pdev; //platform device is a VIP in of_match
	struct vip_shared	*shared;  //shared resource of platform VIP device (between the other SLICE)
	struct resource		*res;     //i.e. shared->res
	struct regmap		*syscon;
	int			instance_id;    //VIP id of this SLICE
	int			slice_id;		//SLICE id
	int			num_ports;	/* count of open ports for SLICE */
	struct mutex		mutex; //open hold this, and also for Q (q->lock)...assigned for each stream of each PORT
	spinlock_t		slock;
	spinlock_t		lock; /* used in videobuf2 callback */

	int			irq;
	int			num_skip_irq;
	void __iomem		*base;   //i.e. shared->base
...
struct vip_port {
	...
	int			num_streams;	/* count of open streams of this PORT */
	...
	struct v4l2_subdev	*subdev;    //single subdevice originally
	struct v4l2_of_endpoint *endpoint;
	struct vip_fmt		*active_fmt[VIP_MAX_ACTIVE_FMT];
	int			num_active_fmt;    //matching format with subdevice and port
};
...
/*
 * VIP Enumerations
 * currently VIP_MULTI_CHANNEL_DATA_SELECT and VIP_RGB_OUT_LO_DATA_SELECT available
 */
...
//p1806 of TRM rev.E, offset 0x0020 from slice0 to find slice1 
