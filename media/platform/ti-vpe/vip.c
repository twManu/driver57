...
//for matching with sub device support format
...
//vip (shared) registers, shared between slices
...
//slice register, shared between ports
 ...
 * masked set bits (mask<<shift) with value field
 */
...
/*
 * with SLICE (vip_dev) mutex hold
 * Enable or disable the SLICE's clocks
 */
...
//reset the data path of the slice
...
//it looks bit28:31 set and all lower bits are 0
			       ...
			       enum data_path_select data_path)
{
	...
	case VIP_MULTI_CHANNEL_DATA_SELECT:
		if (dev->slice_id == VIP_SLICE1) {
			...
			//reg is VIP_VIP1_DATA_PATH_SELECT
...
/*
 * host spin lock to alert vidq -> post_bufs (that DMAing)
 */
static void vip_schedule_next_buffer(struct vip_stream *stream)
{
	...
	if (list_empty(&stream->vidq)) {
		//case: running out of buffer for HW
		vip_dbg(4, dev, "Dropping frame\n");
		if (list_empty(&stream->dropq)) {
			//case: running out of buffer nor dropped buffer, done
			vip_err(dev, "No dropq buffer left!");
			spin_unlock_irqrestore(&dev->slock, flags);
			return;
		}
		/*
		 * case: running out of buffer and there is dropped buffer
		 * - move the first dropped buffer to end of post_bufs
		 * - dma null
		 */
		...
		/*
		 * case: there is buffer in queue and streaming is running,
		 * - clear drop flag
		 * - move the first vidq buffer to end of stream->post_bufs
		 * - to be dma'ed
		 */
		...
		/*
		 * case: there is buffer in queue but streaming stops now
		 */
		vip_err(dev, "IRQ occurred when not streaming\n");
		if (list_empty(&stream->dropq)) {
			//case: no dropped frames, done
			vip_err(dev, "No dropq buffer left!");
			spin_unlock_irqrestore(&dev->slock, flags);
			return;
		}
		/*
		 * - mark it drop
		 * - move the first dropq buffer to end of stream->post_bufs
		 * - dma null
		 */
		buf = list_entry(stream->dropq.next,
				 struct vip_buffer, list);
...
/*
 * - dQ the first buffer from stream->post_bufs and make it done. If the buffer marked drop,
 *   move it to stream->dropq
 * - inc stream sequence
 *
 */
...
static irqreturn_t vip_irq(int irq_vip, void *data)
{
	...
	//locate register for current SLICE, todo optimize before int starts
...
/*
 * with SLICE (vip_dev) mutex hold
 * If it's the first streaming of this port
...
static int vip_init_port(struct vip_port *port)   todo need stream_id
{
	...
	sd_fmt.pad = 0;
	/*
	 * sd_fmt.format is filled with active fourcc format by subdevice
	 * sd_fmt.format.field tells PORT whether it is interlaced
	 *   wxh also reported
	 */
...
/*
 * with SLICE (vip_dev) mutext hold
 * - init when first open to determine frontend format, field, wxh
 * - allocate desc list
 */
static int vip_init_stream(struct vip_stream *stream)
{
	...
	stream->sup_field = f.fmt.pix.field;
	stream->bytesperline = f.fmt.pix.bytesperline;
	stream->sizeimage = f.fmt.pix.sizeimage;   //todo
...
/*
 * allocate a stream, by given stream_id within port, for given port
 * - initialized vb2 queue associated
 * - allocate 3 drop buffer in dropq, init queue of DMAing
 * - allocate videodev and register to system
 */
...
//enumerate sub device formats and enable all matching ones
			      ...
			      struct v4l2_subdev *subdev)
{
	...
	//get matching format by enumerate subdevice formats
			   ...
			   struct v4l2_async_subdev *asd)
{
	...
	port->subdev = subdev;  //single subdev?
...
/*
 * look for each endpoint of PORT, which should be a V4L2 subdevice
 */
				...
				struct device_node *port_node)
{
	...
	//prepare a list of subdevice for given port
...
of_node_cleanup:
	...
	config->asd_sizes = i;
	notifier->bound = vip_async_bound;   //matters
...
/*
 * called after succesfull async firmware download of vip1, 2, or 3 platform device
 * for each port (enabled?), allocate slice/port according <reg> in dev-tree 
 */
static int vip_of_probe(struct platform_device *pdev)
{
	...
	while (1) {
		...
		dev = shared->devs[slice_id]; //retrieve slice device
		dev->syscon = syscon;
...
static int vip_probe(struct platform_device *pdev)
{
	...
	//of_dev_id looks the matched of entry
	...
	/* allocate vip_dev for each slice */
	for (slice = VIP_SLICE1; slice < VIP_NUM_SLICES; slice++) {
		dev = kzalloc(sizeof(*dev), GFP_KERNEL);
		if (!dev)
			return -ENOMEM;

		dev->instance_id = (int)of_dev_id->data;   //the data of matched entry
		...
		//register isr routine for irq number dev->irq
...
/*
 * Refer to arch/arm/mach-omap2/
 *	board-generic.c
 *	omap_hwmod_7xx_data.c
 * 
 * vip1 consists of 2 slice (vip_dev)
 */
static const struct of_device_id vip_of_match[] = {
	{
