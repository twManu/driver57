 ...
 * Allocate a DMA buffer with given size
 */
...
/*
 * with SLICE mutex hold, allocate list with given size
...
int vpdma_create_desc_list(struct vpdma_desc_list *list, size_t size, int type)
{
	...
	list->next = list->buf.addr;   //pointer reset
		...
		int max_w, int max_h, int raw_vpdma_chan, u32 flags)
{
	...
	//rect is copied from c_rect, looks like the coordinate of output buffer
	//i.e. not output to first byte of output buffer
...
/*
 * allocate a slot of vpdma HW list and remember private data
 * Ret : -1 - failure
 *      otherwise - slot index allocated 
 */
...
//invoked when firmware is ready
static void vpdma_firmware_cb(const struct firmware *f, void *context)
{
	 ...
	 * - copy firmware into this area
	 * - map (bi-dir) for DMA engine access
	 */
	...
	//start DMA access as firmware load
	...
	//process fail case and invoke callback if successful
	if (i == 100) {
...
//async prepare and load firmware, and finally invoke callback vpdma->cb when done
...
/*
 * get dmap resource, map register, and then load firmware
 */
